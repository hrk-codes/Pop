import { defineConfig } from 'wxt';

export default defineConfig({
  outDir: 'dist',
  manifest: {
    name: 'POP Chrome Adapter',
    description: 'Shares deliberate web selections and approved X drafts with local POP.',
    version: '0.5.0',
    key: '',
    permissions: ['alarms', 'nativeMessaging'],
    host_permissions: ['http://*/*', 'https://*/*', 'http://127.0.0.1:32145/*'],
    action: { default_title: 'Open POP' },
  },
});
